...
import com.example.dbgt.network.dto.*
...
""".trimIndent()
    ...
    private fun parseResponse(content: String): ModelResponse {
        ...
        // 先检查内容是否包含无效的Type相关模式
        if (content.contains("Type:''") || content.contains("Type:'")) {
            return ModelResponse(
                thinking = "指令格式错误 : ${content.take(200)}",
                action = "finish(message=抱歉，指令格式有误)"
            )
        }
        ...
        if (content.contains("finish(message=")) {
            ...
            if (funcMatch != null) {
                ...
                if (extractedJson.isNotEmpty()) {
                    ...
                    // 如果无法提取有效操作，提供默认错误提示
                    action = "finish(message=抱歉，未能识别有效操作指令)"
                    thinking = "无法解析响应格式 : ${content.take(200)}"
                }
            }
        }
        
        // 再次检查action是否包含无效内容
        if (action.contains("Type:''") || action.contains("Type:'")) {
            return ModelResponse(
                thinking = "指令格式错误",
                action = "finish(message=抱歉，指令格式有误)"
            )
        ...
        if (!action.startsWith("{") && !action.startsWith("do(") && !action.startsWith("finish(")) {
            val funcMatch = Regex("""(do|finish)\s*\([^)]+\)""", RegexOption.IGNORE_CASE).find(action)
            if (funcMatch != null) {
                action = funcMatch.value
            } else {
                // 过滤掉可能的无效内容，如Type:''
        ...
        // 确保action不为空
        if (action.isBlank()) {
            action = "finish(message=未能获取有效操作指令)"
            thinking = "无有效指令 : ${content.take(200)}"
        }
