...
class ActionExecutor(private val service: AutoGLMAccessibilityService) {
    ...
    private fun extractJsonFromText(text: String): String {
        ...
        // 例如：do(action="Tap", element=[389,116]) -> {"_metadata": "do", "action": "Tap", "element": [389, 116]}
    ...
    fun bringAppToForeground() {
        try {
            ...
            if (intent != null) {
                ...
                service.startActivity(intent)
                Log.d("ActionExecutor", "任务完成，正在返回应用: $packageName")
            }
        } catch (e: Exception) {
        ...
        try {
            ...
            for (info in resolveInfos) {
                ...
                if (label.equals(appName, ignoreCase = true) || 
                    ...
                    Log.d("ActionExecutor", "找到匹配的应用: $label -> ${info.activityInfo.packageName}")
                    return info.activityInfo.packageName
                }
            }
        } catch (e: Exception) {
            Log.e("ActionExecutor", "动态查找包名失败", e)
        }
